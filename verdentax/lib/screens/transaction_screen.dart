...
class _TransactionScreenState extends State<TransactionScreen>
          ...
          // Taxe selection
            ...
            Consumer<ConnectivityService>(
              builder: (context, connectivity, _) {
                if (!connectivity.canPerformOnlineOperation()) {
                  return Container(
                    ...
                    child: Row(
                      children: [
                        Icon(Icons.cloud_off, color: Colors.orange, size: 20.sp),
                        SizedBox(width: 8.w),
                        Expanded(
                          child: Text(
                            'Mode hors ligne — la transaction sera synchronisée automatiquement',
  ...
  Future<void> _loadTransactions() async {
    ...
    try {
      ...
      List<TransactionDTO> onlineTransactions = [];
      List<TransactionDTO> offlineTransactions = [];

      if (connectivityService.canPerformOnlineOperation()) {
        try {
          onlineTransactions = await transactionService.getTransactionsByAgent(
            int.tryParse(authService.currentUser?.id ?? '0') ?? 0,
          );
        } catch (e) {
          _logger.w('Could not load online transactions: $e');
        }
      }

      // Always load local offline transactions
      offlineTransactions = await transactionService.getOfflineTransactions();

      _transactions = [...offlineTransactions, ...onlineTransactions];
  ...
  Future<void> _submitTransaction() async {
    ...
    try {
      ...
      final isOffline = !connectivityService.canPerformOnlineOperation();
      ...
      final createdTransaction = await transactionService.createTransaction(
        ...
        contribuableNom: widget.contribuable?.nom,
        contribuablePrenom: widget.contribuable?.prenoms,
        offline: isOffline,
      );
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(isOffline
                ? 'Transaction enregistrée hors ligne (sync automatique au retour de connexion)'
                : 'Transaction ${createdTransaction.numeroRecu} créée'),
