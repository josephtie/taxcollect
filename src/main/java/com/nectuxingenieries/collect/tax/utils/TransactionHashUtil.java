...
public class TransactionHashUtil {
    ...
    public static String generateTransactionHash(Long transactionId, Double montant, Long contribuableId, Long agentId, LocalDateTime timestamp) {
        try {
            String dataToHash = String.format("%d-%.2f-%d-%d-%s", 
                transactionId, montant, contribuableId, agentId, timestamp.format(TIMESTAMP_FORMAT));
    ...
    public record TransactionData(Long transactionId, Double montant, Long contribuableId, Long agentId, LocalDateTime timestamp) {}
}
